...
/**
 * Generates a fake user object with realistic fields for testing.
 * Uses Faker.js to create randomized names, emails, phone numbers, bank info, education info, etc.
 */
function generateUser() {
    ...
    return {
        ...
        dateJoined: faker.date.between({ from: '2022-01-01', to: new Date() })
        ...
        education: { // Nested education info
        ...
        guarantor: { // Nested guarantor info
            fullName: `${faker.person.firstName()} ${faker.person.lastName()}`,
            phoneNumber: `0${faker.number.int({ min: 700000000, max: 909999999 })}`,
            relationship: faker.helpers.arrayElement(['Brother', 'Sister', 'Friend', 'Parent']),
            email: faker.internet.email()
        },
        tier: faker.number.int({ min: 1, max: 3 }) // Tier 1–3
    };
}

/**
 * Generate an array of 500 fake users.
